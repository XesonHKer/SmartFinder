...
[![Release](https://img.shields.io/badge/release-v9.0.0-orange.svg)](#)
[![License](https://img.shields.io/badge/license-MIT-lightgrey.svg)](#license)

[功能特色](#-功能特色) • [PDF 壓縮](#-pdf-壓縮-全新) • [下載安裝](#-下載安裝) • [使用教學](#-使用教學) • [打包說明](#-開發者打包說明) • [更新紀錄](#-更新紀錄-release-notes)
...
> 💡 適用情境：素材整理、檔案歸檔、批量重新命名、跨資料夾檔案管理。

## ✨ 功能特色

- 🗜️ **PDF 壓縮（v9.0.0 新增）**：透過品質拉桿即時預估壓縮後大小，一鍵將 PDF 壓縮至原大小的 15%~70%。內建 Ghostscript 後端，支援背景執行不凍結 UI。
...
|| 5️⃣ | 使用底部按鈕進行操作：打開檔案、打開路徑、批量改名、更改存放位置、**壓縮 PDF**。 |
|| 6️⃣ | （v8.1.0）需要切換語言時，拖曳左下角拉桿即可在 `EN ⇄ 中文` 之間即時切換。 |
|| 7️⃣ | （**v9.0.0 新增**）點擊「壓縮 PDF」→ 選擇檔案 → 拖動品質拉桿預估大小 → 按壓縮。需先安裝 Ghostscript：`brew install ghostscript` |
...
- **芯片要求**：Apple M1（Intel 也可）
- **Python**：3.9+（從原始碼執行時）
- **相依套件**：PyQt5
- **PDF 壓縮**：Ghostscript（選用，`brew install ghostscript`）

---

## 📋 更新紀錄 (Release Notes)

詳見 [RELEASE_NOTES.md](RELEASE_NOTES.md) 或 [Releases 頁面](../../releases)。

|最新版本 **v9.0.0** 重點（2026-05-11）：
|- 🗜️ **PDF 壓縮功能** — 品質拉桿即時預估大小，背景執行不凍結 UI。需 `brew install ghostscript`
|- 🌐 原有 **English / 繁體中文** 雙語介面完整保留
|- 🎚 原有語言切換拉桿（`EN ⇄ 中文`）及偏好持久化功能完整保留
|- 📦 V9.0.0 原始碼與打包腳本獨立放置於 `V9.0.0/` 目錄，原始 V8 / V8.1.0 內容完整保留
...
**Q6：（v9.0.0）PDF 壓縮需要安裝什麼？**  
A：需要 **Ghostscript**。在終端機執行：`brew install ghostscript`。若未安裝，點擊「壓縮 PDF」時會自動提示。

**Q7：如果不想安裝 Ghostscript，其他功能還能用嗎？**  
A：完全可以。所有搜尋、改名、搬移功能都不受影響，僅「壓縮 PDF」按鈕需要 Ghostscript。
