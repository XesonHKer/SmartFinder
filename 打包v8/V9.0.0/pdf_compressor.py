...
def estimate_pdf_size(pdf_path, quality_pct):
    ...
    try:
        ...
        ratios = {
            0: 0.15, 10: 0.18, 20: 0.22, 25: 0.25,
            30: 0.30, 40: 0.35, 50: 0.40, 60: 0.50,
            70: 0.60, 75: 0.65, 80: 0.72, 90: 0.85, 100: 1.0,
        }
        closest_pct = min(ratios.keys(), key=lambda x: abs(x - quality_pct))
        estimated = int(size_bytes * ratios[closest_pct])
        return estimated
    except:
        return 0
...
class PdfCompressDialog(QDialog):
    ...
    def start_compress(self):
        ...
        # Build command (no -dQUIET so we get real-time stderr progress)
        cmd = [
            gs_path,
            "-sDEVICE=pdfwrite",
