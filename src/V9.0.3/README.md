...
[![Release](https://img.shields.io/badge/release-v9.0.3-orange.svg)](#)
[![License](https://img.shields.io/badge/license-MIT-lightgrey.svg)](#license)

[功能特色](#-功能特色) • [PDF 壓縮](#-pdf-壓縮) • [下載安裝](#-下載安裝) • [使用教學](#-使用教學) • [打包說明](#-開發者打包說明) • [更新紀錄](#-更新紀錄-release-notes)
...
- 🗜️ **PDF 壓縮（v9.0.0 新增）**：透過品質拉桿即時預估壓縮後大小，一鍵將 PDF 壓縮至原大小的 15%~70%。
- 🚀 **Ghostscript 內建（v9.0.2 新增）**：不再需要手動安裝 Ghostscript！GS 已打包進 `.app` 內部，下載即用。
...
1. 前往 [Releases](../../releases) 頁面下載最新版 `SmartFinder-v9.0.3-macOS-arm64.zip`。
2. 雙擊 `.zip` 檔案解壓縮。
3. 將 `SmartFinder.app` 拖曳至「應用程式」資料夾。
4. 首次開啟若出現「無法打開，因為它來自未識別的開發者」：
   - 前往「系統設定 → 隱私權與安全性」，點擊 **仍要打開**；或
   - 在終端機執行：`xattr -dr com.apple.quarantine /Applications/SmartFinder.app`
5. **PDF 壓縮功能開箱即用**，無需額外安裝任何東西！
   - （v9.0.2 起 Ghostscript 已內建於 .app 中）
...
|| 7️⃣ | （v9.0.0 新增）點擊「壓縮 PDF」→ 選擇檔案 → 拖動品質拉桿預估大小 → 按壓縮。 |
...
- **芯片要求**：Apple Silicon (M1/M2/M3/M4) 或 Intel
- **Python**：3.9+（從原始碼執行時）
- **相依套件**：PyQt5
- **PDF 壓縮**：自 v9.0.2 起已內建 Ghostscript，無需安裝
...
|最新版本 **v9.0.2**（2026-05-11）重點：
|||- 🚀 **Ghostscript 內建打包** — 不再需要 `brew install ghostscript`，下載即用 PDF 壓縮！
|||- 🗜️ PDF 壓縮支援取消按鈕，並顯示品質設定與即時進度
|||- 📦 V9.0.2 原始碼含 GS Bundle 完整打包，檔案大小約 28 MB

**v9.0.1**（2026-05-11）重點：
|||- 🐛 **修復 PDF 壓縮 UI「無回應」問題** — 改用 QProcess 異步執行，加入即時進度顯示與取消按鈕
|||- 🏷 **左下角新增版本號標記** — 淺灰色 `v9.0.1` 顯示於介面左下角
|||- 🗜️ PDF 壓縮功能完整保留
...
A：**v9.0.2 起已內建 Ghostscript**，無需任何安裝！若使用 v9.0.1 或更舊版本，請執行 `brew install ghostscript`。

**Q7：如果不想安裝 / 使用 PDF 壓縮功能？**  
A：完全可以。所有搜尋、改名、搬移功能都不受影響。
