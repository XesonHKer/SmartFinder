...
# =========================================================
#  Import PDF Compressor module
# =========================================================
from pdf_compressor import PdfCompressDialog
...
class SmartFinderWindow(QMainWindow):
    def __init__(self):
        try:
            ...
            # 排序：點擊欄位可排序（asc/desc toggle）
            self.result_listbox.setSortingEnabled(True)
            self.result_listbox.sortByColumn(0, Qt.AscendingOrder)
            self._sort_col = 0
            self._sort_order = Qt.AscendingOrder
            self.result_listbox.header().sectionClicked.connect(self._on_header_clicked)
            ...
            self.compress_pdf_button = QPushButton()
            self.compress_pdf_button.clicked.connect(self.open_pdf_compressor)
            ...
            action_layout.addWidget(self.compress_pdf_button)
            ...
            footer_layout.addWidget(self.version_label, 0, Qt.AlignVCenter)
    ...
    def retranslate_ui(self):
        try:
            ...
            self.compress_pdf_button.setText(self.t("compress_pdf"))
    ...
    def _on_header_clicked(self, col):
        """Toggle sort: same column toggles asc/desc, different column starts asc."""
        if col == self._sort_col:
            self._sort_order = Qt.DescendingOrder if self._sort_order == Qt.AscendingOrder else Qt.AscendingOrder
        else:
            self._sort_col = col
            self._sort_order = Qt.AscendingOrder
        self.result_listbox.sortByColumn(col, self._sort_order)
    ...
    def open_pdf_compressor(self):
        try:
            dialog = PdfCompressDialog(self, lang=self.lang)
            dialog.exec_()
        except Exception:
            pass
